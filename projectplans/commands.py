...
class UpdateOverlapToleranceCommand(QUndoCommand):
    def __init__(self, model, value):
        super().__init__("Change Overlap Layout")
        self.model = model
        self.old = model.symbol_overlap_tolerance_percent
        self.new = value

    def _apply(self, value):
        self.model.symbol_overlap_tolerance_percent = value
        self.model.objects_changed.emit()
...
class AddObjectCommand(QUndoCommand):
    ...
    def mergeWith(self, other):
        if isinstance(other, UpdateObjectCommand) and other.initial_name and other.old_obj == self.obj:
            self.obj = other.new_obj
            return True
        return False
...
class UpdateObjectCommand(QUndoCommand):
    def id(self):
        return 71001 if self.initial_name else -1

    def __init__(self, model, old_obj, new_obj, description: str, initial_name=False) -> None:
        super().__init__(description)
        self.model = model
...
class ReplaceTopicsCommand(QUndoCommand):
    def __init__(
        self,
        model,
        old_topics,
        new_topics,
        description: str = "Update Deliverables",
        removed_objects=None,
    ) -> None:
        super().__init__(description)
        self.model = model
        self.old_topics = deepcopy(old_topics)
        self.new_topics = deepcopy(new_topics)
        self.removed_objects = deepcopy(removed_objects or [])

    def redo(self) -> None:
        for obj in self.removed_objects:
            self.model.remove_object(obj.id)
        self.model.set_topics(deepcopy(self.new_topics))

    def undo(self) -> None:
        self.model.set_topics(deepcopy(self.old_topics))
        for obj in self.removed_objects:
            self.model.add_object(deepcopy(obj))


class SetSubjectsCommand(QUndoCommand):
    def __init__(self, model, old_subjects, new_subjects, description="Update Subjects") -> None:
        super().__init__(description)
        self.model = model
        self.old_subjects = deepcopy(old_subjects)
        self.new_subjects = deepcopy(new_subjects)

    def redo(self) -> None:
        self.model.set_subjects(deepcopy(self.new_subjects))

    def undo(self) -> None:
        self.model.set_subjects(deepcopy(self.old_subjects))


class SetInitiativesCommand(QUndoCommand):
    def __init__(self, model, old_items, new_items, description="Update Initiatives") -> None:
        super().__init__(description)
        self.model = model
        self.old_items = deepcopy(old_items)
        self.new_items = deepcopy(new_items)

    def redo(self) -> None:
        self.model.set_initiatives(deepcopy(self.new_items))

    def undo(self) -> None:
        self.model.set_initiatives(deepcopy(self.old_items))
