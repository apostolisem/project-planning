from __future__ import annotations

from copy import deepcopy
from dataclasses import replace

from PyQt6.QtGui import QUndoStack

from .commands import (
    AddObjectCommand,
    UpdateOverlapToleranceCommand,
    AddTopicCommand,
    RemoveObjectCommand,
    RemoveTopicCommand,
    ReplaceTopicsCommand,
    SetInitiativesCommand,
    SetSubjectsCommand,
    ToggleTopicCollapseCommand,
    UpdateClassificationCommand,
    UpdateObjectCommand,
    UpdateDeliverableCommand,
    UpdateTopicCommand,
)
from .constants import (
    CANVAS_ROW_ID,
    DEFAULT_SIZE,
    DEADLINE_DEFAULT_COLOR,
    CONNECTOR_DEFAULT_COLOR,
    LINK_LINE_COLOR,
    TEXTBOX_DEFAULT_COLOR,
    TEXTBOX_DEFAULT_OPACITY,
    TEXTBOX_MIN_HEIGHT,
    TEXTBOX_MIN_WIDTH,
    TEXTBOX_NEW_OPACITY,
)
from .layout import row_item_height, row_item_size
from .model import (
    CanvasObject,
    Deliverable,
    Initiative,
    ProjectModel,
    ProjectSubject,
    Topic,
    new_id,
    DEFAULT_TOPIC_COLORS,
    normalize_arrow_direction,
)


class ProjectController:
    def __init__(self, model: ProjectModel, undo_stack: QUndoStack) -> None:
        self.model = model
        self.undo_stack = undo_stack
        self.layout = None
        self.connector_default_size = 1
        self.connector_default_color = CONNECTOR_DEFAULT_COLOR

    def set_layout(self, layout) -> None:
        self.layout = layout

    @staticmethod
    def _normalize_deliverables(deliverables: list[Deliverable]) -> list[Deliverable]:
        normalized: list[Deliverable] = []
        previous_indent = 0
        for index, deliverable in enumerate(deliverables):
            try:
                indent = int(getattr(deliverable, "indent", 0))
            except (TypeError, ValueError):
                indent = 0
            max_indent = 0 if index == 0 else previous_indent + 1
            indent = max(0, min(indent, max_indent))
            updated = replace(deliverable, indent=indent)
            normalized.append(updated)
            previous_indent = indent
        return normalized

    @classmethod
    def _normalize_topics(
        cls, topics: list[Topic], topic_ids: set[str] | None = None
    ) -> list[Topic]:
        if topic_ids is None:
            topic_ids = {topic.id for topic in topics}
        normalized_topics: list[Topic] = []
        for topic in topics:
            if topic.id not in topic_ids:
                normalized_topics.append(topic)
                continue
            normalized_topics.append(
                replace(topic, deliverables=cls._normalize_deliverables(topic.deliverables))
            )
        return normalized_topics

    @staticmethod
    def _topic_index(topics: list[Topic], topic_id: str) -> int | None:
        for index, topic in enumerate(topics):
            if topic.id == topic_id:
                return index
        return None

    @staticmethod
    def _find_deliverable_in_topics(
        topics: list[Topic], deliverable_id: str
    ) -> tuple[int, int, Deliverable] | None:
        for topic_index, topic in enumerate(topics):
            for deliverable_index, deliverable in enumerate(topic.deliverables):
                if deliverable.id == deliverable_id:
                    return topic_index, deliverable_index, deliverable
        return None

    @staticmethod
    def _ordered_deliverable_ids(topics: list[Topic], deliverable_ids: list[str]) -> list[str]:
        wanted = set(deliverable_ids)
        ordered: list[str] = []
        for topic in topics:
            for deliverable in topic.deliverables:
                if deliverable.id in wanted:
                    ordered.append(deliverable.id)
        return ordered

    def _move_deliverable_in_topics(
        self, topics: list[Topic], deliverable_id: str, direction: int
    ) -> set[str]:
        found = self._find_deliverable_in_topics(topics, deliverable_id)
        if found is None:
            return set()
        topic_index, deliverable_index, deliverable = found
        source_topic = topics[topic_index]

        if direction < 0:
            if deliverable_index > 0:
                source_topic.deliverables.pop(deliverable_index)
                source_topic.deliverables.insert(deliverable_index - 1, deliverable)
                return {source_topic.id}
            if topic_index == 0:
                return set()
            target_topic = topics[topic_index - 1]
            source_topic.deliverables.pop(deliverable_index)
            target_topic.deliverables.insert(len(target_topic.deliverables), deliverable)
            return {source_topic.id, target_topic.id}

        if direction > 0:
            if deliverable_index < len(source_topic.deliverables) - 1:
                source_topic.deliverables.pop(deliverable_index)
                source_topic.deliverables.insert(deliverable_index + 1, deliverable)
                return {source_topic.id}
            if topic_index >= len(topics) - 1:
                return set()
            target_topic = topics[topic_index + 1]
            source_topic.deliverables.pop(deliverable_index)
            target_topic.deliverables.insert(0, deliverable)
            return {source_topic.id, target_topic.id}

        return set()

    def _object_anchor_point(self, obj: CanvasObject) -> tuple[float, float] | None:
        layout = self.layout
        if layout is None:
            return None
        if obj.kind == "textbox":
            width = obj.width if obj.width is not None else TEXTBOX_MIN_WIDTH
            height = obj.height if obj.height is not None else TEXTBOX_MIN_HEIGHT
            x = obj.x if obj.x is not None else 0.0
            y = obj.y if obj.y is not None else 0.0
            return (x + (width / 2.0), y + (height / 2.0))
        if obj.kind == "milestone":
            if obj.row_id not in layout.row_map:
                return None
            row_height = layout.row_height(obj.row_id)
            size = row_item_size(row_height, obj.size, layout.week_width)
            center_x = layout.week_right_x(obj.end_week)
            center_y = layout.row_center_y(obj.row_id) + layout.object_vertical_offset(obj.id)
            return (center_x, center_y)
        if obj.kind == "circle":
            if obj.row_id not in layout.row_map:
                return None
            row_height = layout.row_height(obj.row_id)
            size = row_item_size(row_height, obj.size, layout.week_width)
            center_x = layout.week_center_x(obj.start_week)
            center_y = layout.row_center_y(obj.row_id)
            return (center_x, center_y)
        if obj.kind == "deadline":
            center_x = layout.week_right_x(obj.end_week)
            center_y = layout.header_height + (layout.total_height / 2.0)
            return (center_x, center_y)
        if obj.kind in ("connector", "arrow") and not (
            obj.connector_source_id and obj.connector_target_id
        ):
            if obj.row_id not in layout.row_map:
                return None
            target_row = obj.target_row_id or obj.row_id
            if target_row not in layout.row_map:
                return None
            start_x = layout.week_center_x(obj.start_week)
            start_y = layout.row_center_y(obj.row_id)
            target_week = obj.target_week if obj.target_week is not None else obj.end_week
            end_x = layout.week_center_x(target_week)
            end_y = layout.row_center_y(target_row)
            return ((start_x + end_x) / 2.0, (start_y + end_y) / 2.0)
        if obj.row_id not in layout.row_map:
            return None
        row_height = layout.row_height(obj.row_id)
        height = row_item_height(row_height, obj.size)
        width = max(1, obj.end_week - obj.start_week + 1) * layout.week_width
        x = layout.week_left_x(obj.start_week)
        y = layout.row_top_y(obj.row_id) + ((row_height - height) / 2.0)
        return (x + (width / 2.0), y + (height / 2.0))

    @staticmethod
    def _textbox_anchor_point(
        obj: CanvasObject, side: str | None, offset: float | None
    ) -> tuple[float, float]:
        width = obj.width if obj.width is not None else TEXTBOX_MIN_WIDTH
        height = obj.height if obj.height is not None else TEXTBOX_MIN_HEIGHT
        x = obj.x if obj.x is not None else 0.0
        y = obj.y if obj.y is not None else 0.0
        offset_value = float(offset or 0.5)
        if offset_value < 0.0:
            offset_value = 0.0
        if offset_value > 1.0:
            offset_value = 1.0
        if side == "left":
            return (x, y + (height * offset_value))
        if side == "top":
            return (x + (width * offset_value), y)
        if side == "bottom":
            return (x + (width * offset_value), y + height)
        return (x + width, y + (height * offset_value))

    def _links_from_source(self, source_id: str) -> list[CanvasObject]:
        return [
            obj
            for obj in self.model.objects.values()
            if obj.kind == "link" and obj.link_source_id == source_id
        ]

    def _clamp_week(self, week: int) -> int:
        return int(week)

    def _next_z_index(self) -> int:
        if not self.model.objects:
            return 0
        return max(obj.z_index for obj in self.model.objects.values()) + 1

    def _normalize_object(self, obj: CanvasObject) -> CanvasObject:
        start_week = self._clamp_week(obj.start_week)
        end_week = self._clamp_week(obj.end_week)
        size = max(1, min(5, obj.size))
        z_index = int(obj.z_index)
        opacity = float(obj.opacity)
        text_html = obj.text_html
        if text_html == "":
            text_html = None
        notes_html = obj.notes_html
        if notes_html == "":
            notes_html = None
        scope_html = obj.scope_html
        if scope_html == "":
            scope_html = None
        risks_html = obj.risks_html
        if risks_html == "":
            risks_html = None
        arrow_direction = normalize_arrow_direction(getattr(obj, "arrow_direction", "none"))
        if obj.kind != "box":
            arrow_direction = "none"
        obj = replace(
            obj,
            text_html=text_html,
            notes_html=notes_html,
            scope_html=scope_html,
            risks_html=risks_html,
            arrow_direction=arrow_direction,
        )
        if opacity < 0.0:
            opacity = 0.0
        if opacity > 1.0:
            opacity = 1.0
        arrow_head_start = bool(getattr(obj, "arrow_head_start", False))
        arrow_head_end = bool(getattr(obj, "arrow_head_end", True))
        if obj.kind in ("arrow", "connector") and not (arrow_head_start or arrow_head_end):
            arrow_head_end = True

        if obj.kind in ("milestone", "circle", "deadline"):
            end_week = start_week
        elif obj.kind == "link":
            link_offset_x = float(obj.link_offset_x or 0.0)
            link_offset_y = float(obj.link_offset_y or 0.0)
            link_source_offset = obj.link_source_offset
            if link_source_offset is not None:
                link_source_offset = float(link_source_offset)
                link_source_offset = max(0.0, min(1.0, link_source_offset))
            return replace(
                obj,
                start_week=start_week,
                end_week=end_week,
                size=size,
                z_index=z_index,
                opacity=opacity,
                link_offset_x=link_offset_x,
                link_offset_y=link_offset_y,
                link_source_offset=link_source_offset,
            )
        elif obj.kind in ("connector", "arrow"):
            # Unified connector: free mode mirrors target_week into end_week.
            target_week = obj.target_week
            if target_week is not None:
                target_week = self._clamp_week(target_week)
                end_week = target_week
            source_offset = obj.connector_source_offset
            if source_offset is not None:
                source_offset = float(source_offset)
                source_offset = max(0.0, min(1.0, source_offset))
            target_offset = obj.connector_target_offset
            if target_offset is not None:
                target_offset = float(target_offset)
                target_offset = max(0.0, min(1.0, target_offset))
            return replace(
                obj,
                start_week=start_week,
                end_week=end_week,
                size=size,
                z_index=z_index,
                opacity=opacity,
                target_week=target_week,
                connector_source_offset=source_offset,
                connector_target_offset=target_offset,
                arrow_head_start=arrow_head_start,
                arrow_head_end=arrow_head_end,
            )
        elif obj.kind == "textbox":
            x = float(obj.x or 0.0)
            y = float(obj.y or 0.0)
            width = float(obj.width or TEXTBOX_MIN_WIDTH)
            height = float(obj.height or TEXTBOX_MIN_HEIGHT)
            if width < TEXTBOX_MIN_WIDTH:
                width = TEXTBOX_MIN_WIDTH
            if height < TEXTBOX_MIN_HEIGHT:
                height = TEXTBOX_MIN_HEIGHT
            return replace(
                obj,
                start_week=start_week,
                end_week=end_week,
                size=size,
                z_index=z_index,
                x=x,
                y=y,
                width=width,
                height=height,
                opacity=opacity,
                arrow_head_start=arrow_head_start,
                arrow_head_end=arrow_head_end,
            )
        else:
            if end_week < start_week:
                end_week = start_week

        return replace(
            obj,
            start_week=start_week,
            end_week=end_week,
            size=size,
            z_index=z_index,
            opacity=opacity,
            arrow_head_start=arrow_head_start,
            arrow_head_end=arrow_head_end,
        )

    def _duplicate_offset(self, obj: CanvasObject) -> int:
        end_week = obj.target_week if obj.target_week is not None else obj.end_week
        span = abs(end_week - obj.start_week) + 1
        return max(1, span)

    def add_object(self, obj: CanvasObject, description: str = "Add Object") -> None:
        if obj.z_index == 0 and self.model.objects:
            obj = replace(obj, z_index=self._next_z_index())
        obj = self._normalize_object(obj)
        self.undo_stack.push(AddObjectCommand(self.model, obj, description))

    def name_created_object(self, obj_id, text, html):
        obj = self.model.objects.get(obj_id)
        if not obj:
            return
        updated = self._normalize_object(replace(obj, text=text, text_html=html))
        if updated != obj:
            self.undo_stack.push(UpdateObjectCommand(self.model, obj, updated, "Name Object", initial_name=True))

    def remove_object(self, obj_id: str) -> None:
        self.remove_objects([obj_id])

    def remove_objects(self, ids):
        removed = set(ids) & self.model.objects.keys()
        changed = True
        while changed:
            changed = False
            for obj in self.model.objects.values():
                endpoints = (obj.link_source_id, obj.link_target_id, obj.connector_source_id, obj.connector_target_id)
                if obj.id not in removed and any(endpoint in removed for endpoint in endpoints if endpoint):
                    removed.add(obj.id)
                    changed = True
        if not removed:
            return
        self.undo_stack.beginMacro("Delete selection")
        try:
            for obj in list(self.model.objects.values()):
                if obj.id in removed:
                    self.undo_stack.push(RemoveObjectCommand(self.model, obj))
        finally:
            self.undo_stack.endMacro()

    def update_objects(self, ids, changes, description):
        objects = [self.model.objects[i] for i in ids if i in self.model.objects]
        objects = [o for o in objects if self._normalize_object(replace(o, **changes)) != o]
        if not objects:
            return
        self.undo_stack.beginMacro(description)
        try:
            for obj in objects:
                self.update_object(obj.id, changes, description)
        finally:
            self.undo_stack.endMacro()

    def duplicate_objects(self, ids):
        relationship_kinds = {"link", "connector", "arrow"}
        selected = {
            obj_id
            for obj_id in set(ids) & self.model.objects.keys()
            if self.model.objects[obj_id].kind not in relationship_kinds
        }
        if len(selected) == 1:
            clone = self.duplicate_object(next(iter(selected)))
            return [clone.id] if clone else []
        if not selected:
            return []
        offset = self.layout.week_width if self.layout else 40
        clones = []
        for obj in list(self.model.objects.values()):
            if obj.id not in selected:
                continue
            changes = {"id": new_id(), "start_week": self._clamp_week(obj.start_week + 1),
                       "end_week": self._clamp_week(obj.end_week + 1), "z_index": self._next_z_index() + len(clones)}
            for key in ("target_week", "arrow_mid_week"):
                if getattr(obj, key) is not None:
                    changes[key] = self._clamp_week(getattr(obj, key) + 1)
            if obj.kind == 'textbox':
                changes['x'] = (obj.x or 0) + offset
            changes.update(self._relationship_free_changes())
            clones.append(replace(obj, **changes))
        self.undo_stack.beginMacro("Duplicate selection")
        try:
            for obj in clones:
                self.add_object(obj, "Duplicate Object")
        finally:
            self.undo_stack.endMacro()
        return [o.id for o in clones]

    @staticmethod
    def _relationship_free_changes() -> dict:
        return {
            "predecessors": [],
            "link_source_id": None,
            "link_target_id": None,
            "link_source_side": None,
            "link_source_offset": None,
            "link_offset_x": None,
            "link_offset_y": None,
            "connector_source_id": None,
            "connector_target_id": None,
            "connector_source_side": None,
            "connector_source_offset": None,
            "connector_target_side": None,
            "connector_target_offset": None,
        }

    def update_object(
        self,
        obj_id: str,
        changes: dict,
        description: str,
        *,
        skip_anchor_sources: set[str] | None = None,
        defer_link_updates: bool = False,
    ) -> None:
        # Kept in the public call shape for grouped movement callers. Target
        # changes never reposition textbox sources; only explicit textbox
        # movement refreshes link anchor metadata.
        obj = self.model.objects.get(obj_id)
        if obj is None:
            return
        new_obj = replace(obj, **changes)
        new_obj = self._normalize_object(new_obj)
        if new_obj == obj:
            return
        if obj.kind in ("connector", "arrow") and "size" in changes:
            self.connector_default_size = new_obj.size
        if obj.kind in ("connector", "arrow") and "color" in changes:
            self.connector_default_color = new_obj.color
        source_links = self._links_from_source(obj_id) if obj.kind == "textbox" else []

        updates: list[tuple[CanvasObject, CanvasObject, str]] = [(obj, new_obj, description)]

        link_updates: list[tuple[CanvasObject, CanvasObject]] = []
        if source_links and not defer_link_updates:
            for link in source_links:
                target_id = link.link_target_id
                if not target_id:
                    continue
                target = self.model.objects.get(target_id)
                if target is None:
                    continue
                target_point = self._object_anchor_point(target)
                if target_point is None:
                    continue
                source_anchor = self._textbox_anchor_point(
                    new_obj, link.link_source_side, link.link_source_offset
                )
                new_offset_x = source_anchor[0] - target_point[0]
                new_offset_y = source_anchor[1] - target_point[1]
                old_offset_x = float(link.link_offset_x or 0.0)
                old_offset_y = float(link.link_offset_y or 0.0)
                if abs(new_offset_x - old_offset_x) > 0.01 or abs(new_offset_y - old_offset_y) > 0.01:
                    new_link = replace(
                        link,
                        link_offset_x=new_offset_x,
                        link_offset_y=new_offset_y,
                    )
                    link_updates.append((link, self._normalize_object(new_link)))

        if len(updates) > 1 or link_updates:
            self.undo_stack.beginMacro(description)
            for old_obj, updated_obj, label in updates:
                self.undo_stack.push(UpdateObjectCommand(self.model, old_obj, updated_obj, label))
            for old_link, new_link in link_updates:
                self.undo_stack.push(UpdateObjectCommand(self.model, old_link, new_link, "Update Anchor"))
            self.undo_stack.endMacro()
            self._reschedule_dependents(new_obj.id)
            return
        self.undo_stack.push(UpdateObjectCommand(self.model, obj, new_obj, description))
        self._reschedule_dependents(new_obj.id)

    def _reschedule_dependents(self, changed_id: str) -> None:
        """Push finish-to-start successor moves after a date change."""
        if not self.model.dependencies_auto_reschedule:
            return
        if changed_id not in self.model.objects:
            return
        self.undo_stack.beginMacro("Auto-reschedule Dependencies")
        visiting: set[str] = set()
        moved: set[str] = set()

        def walk(pred_id: str) -> None:
            if pred_id in visiting:
                return
            visiting.add(pred_id)
            pred = self.model.objects.get(pred_id)
            if pred is None:
                visiting.discard(pred_id)
                return
            for successor in list(self.model.objects.values()):
                if pred_id not in (successor.predecessors or []):
                    continue
                required = max(
                    [self.model.objects[p].end_week + 1 for p in successor.predecessors
                     if p in self.model.objects] or [successor.start_week]
                )
                if successor.start_week < required:
                    duration = max(0, successor.end_week - successor.start_week)
                    new_start = self._clamp_week(required)
                    updated = replace(
                        successor,
                        start_week=new_start,
                        end_week=self._clamp_week(new_start + duration),
                    )
                    updated = self._normalize_object(updated)
                    self.undo_stack.push(UpdateObjectCommand(
                        self.model, successor, updated, "Auto-reschedule Dependency"
                    ))
                    moved.add(successor.id)
                walk(successor.id)
            visiting.discard(pred_id)

        try:
            walk(changed_id)
        finally:
            self.undo_stack.endMacro()

    def refresh_anchor_offsets(self, source_ids: set[str], description: str = "Update Anchors") -> None:
        if not source_ids:
            return
        if self.layout is None:
            return
        link_updates: list[tuple[CanvasObject, CanvasObject]] = []
        for source_id in source_ids:
            source = self.model.objects.get(source_id)
            if source is None or source.kind != "textbox":
                continue
            for link in self._links_from_source(source_id):
                target_id = link.link_target_id
                if not target_id:
                    continue
                target = self.model.objects.get(target_id)
                if target is None:
                    continue
                target_point = self._object_anchor_point(target)
                if target_point is None:
                    continue
                source_anchor = self._textbox_anchor_point(
                    source, link.link_source_side, link.link_source_offset
                )
                new_offset_x = source_anchor[0] - target_point[0]
                new_offset_y = source_anchor[1] - target_point[1]
                old_offset_x = float(link.link_offset_x or 0.0)
                old_offset_y = float(link.link_offset_y or 0.0)
                if abs(new_offset_x - old_offset_x) > 0.01 or abs(new_offset_y - old_offset_y) > 0.01:
                    new_link = replace(
                        link,
                        link_offset_x=new_offset_x,
                        link_offset_y=new_offset_y,
                    )
                    link_updates.append((link, self._normalize_object(new_link)))
        if not link_updates:
            return
        self.undo_stack.beginMacro(description)
        for old_link, new_link in link_updates:
            self.undo_stack.push(UpdateObjectCommand(self.model, old_link, new_link, "Update Anchor"))
        self.undo_stack.endMacro()

    def duplicate_object(self, obj_id: str) -> CanvasObject | None:
        obj = self.model.objects.get(obj_id)
        if obj is None or obj.kind in {"link", "connector", "arrow"}:
            return None
        if obj.kind == "textbox":
            width = obj.width or TEXTBOX_MIN_WIDTH
            padding = max(10.0, width * 0.1)
            new_x = (obj.x or 0.0) + width + padding
            cloned = replace(
                obj,
                id=new_id(),
                x=new_x,
                z_index=self._next_z_index(),
                **self._relationship_free_changes(),
            )
            self.add_object(cloned, "Duplicate Object")
            return cloned
        delta = self._duplicate_offset(obj)
        new_start = self._clamp_week(obj.start_week + delta)
        new_end = self._clamp_week(obj.end_week + delta)
        target_week = obj.target_week + delta if obj.target_week is not None else None
        if target_week is not None:
            target_week = self._clamp_week(target_week)
        arrow_mid_week = obj.arrow_mid_week + delta if obj.arrow_mid_week is not None else None
        if arrow_mid_week is not None:
            arrow_mid_week = self._clamp_week(arrow_mid_week)
        cloned = replace(
            obj,
            id=new_id(),
            start_week=new_start,
            end_week=new_end,
            target_week=target_week,
            arrow_mid_week=arrow_mid_week,
            z_index=self._next_z_index(),
            **self._relationship_free_changes(),
        )
        self.add_object(cloned, "Duplicate Object")
        return cloned

    def add_topic(
        self,
        name: str,
        color: str | None = None,
        *,
        index: int | None = None,
        description: str = "Add Topic",
    ) -> Topic:
        if color is None:
            color = DEFAULT_TOPIC_COLORS[len(self.model.topics) % len(DEFAULT_TOPIC_COLORS)]
        topic = Topic(id=new_id(), name=name, color=color)
        self.undo_stack.push(AddTopicCommand(self.model, topic, index=index, description=description))
        return topic

    # --- Subjects / initiatives -------------------------------------------
    def add_subject(self, name: str, color: str | None = None) -> ProjectSubject:
        subjects = list(self.model.subjects)
        if color is None:
            color = DEFAULT_TOPIC_COLORS[len(subjects) % len(DEFAULT_TOPIC_COLORS)]
        subject = ProjectSubject(id=new_id(), name=name, color=color)
        self.undo_stack.push(
            SetSubjectsCommand(self.model, subjects, subjects + [subject], "Add Subject")
        )
        return subject

    def update_subject(self, subject_id: str, *, name: str | None = None,
                       color: str | None = None) -> None:
        subjects = list(self.model.subjects)
        changed = False
        new_subjects = []
        for subject in subjects:
            if subject.id == subject_id:
                updated = replace(
                    subject,
                    name=name if name is not None else subject.name,
                    color=color if color is not None else subject.color,
                )
                changed = changed or updated != subject
                new_subjects.append(updated)
            else:
                new_subjects.append(subject)
        if changed:
            self.undo_stack.push(
                SetSubjectsCommand(self.model, subjects, new_subjects, "Update Subject")
            )

    def remove_subject(self, subject_id: str) -> None:
        subjects = list(self.model.subjects)
        new_subjects = [s for s in subjects if s.id != subject_id]
        if len(new_subjects) == len(subjects):
            return
        self.undo_stack.beginMacro("Remove Subject")
        self.undo_stack.push(
            SetSubjectsCommand(self.model, subjects, new_subjects, "Remove Subject")
        )
        for obj in list(self.model.objects.values()):
            if obj.subject_id == subject_id:
                self.update_object(obj.id, {"subject_id": None}, "Clear Subject")
        self.undo_stack.endMacro()

    def assign_subject(self, obj_id: str, subject_id: str | None) -> None:
        obj = self.model.objects.get(obj_id)
        if obj is None:
            return
        if obj.subject_id == subject_id:
            return
        subject = self.model.get_subject(subject_id)
        changes: dict = {"subject_id": subject_id}
        if subject is not None:
            changes["color"] = subject.color
        self.update_object(obj_id, changes, "Assign Subject")

    def add_initiative(self, name: str, color: str | None = None,
                       row_id: str | None = None) -> Initiative:
        items = list(self.model.initiatives)
        if color is None:
            color = DEFAULT_TOPIC_COLORS[len(items) % len(DEFAULT_TOPIC_COLORS)]
        initiative = Initiative(id=new_id(), name=name, color=color, row_id=row_id)
        self.undo_stack.push(
            SetInitiativesCommand(
                self.model, items, items + [initiative], "Add Initiative"
            )
        )
        return initiative

    def remove_initiative(self, initiative_id: str) -> None:
        items = list(self.model.initiatives)
        new_items = [i for i in items if i.id != initiative_id]
        if len(new_items) != len(items):
            self.undo_stack.push(
                SetInitiativesCommand(self.model, items, new_items, "Remove Initiative")
            )

    def link_to_initiative(self, initiative_id: str, obj_ids) -> None:
        items = list(self.model.initiatives)
        target = next((i for i in items if i.id == initiative_id), None)
        if target is None:
            return
        members = sorted(set(target.member_ids) | {str(o) for o in obj_ids})
        if members == list(target.member_ids):
            return
        new_items = [
            replace(i, member_ids=members) if i.id == initiative_id else i
            for i in items
        ]
        self.undo_stack.push(
            SetInitiativesCommand(self.model, items, new_items, "Link to Initiative")
        )

    def unlink_from_initiative(self, initiative_id: str, obj_ids) -> None:
        items = list(self.model.initiatives)
        target = next((i for i in items if i.id == initiative_id), None)
        if target is None:
            return
        remove = {str(o) for o in obj_ids}
        members = [m for m in target.member_ids if m not in remove]
        if members == list(target.member_ids):
            return
        new_items = [
            replace(i, member_ids=members) if i.id == initiative_id else i
            for i in items
        ]
        self.undo_stack.push(
            SetInitiativesCommand(self.model, items, new_items, "Unlink from Initiative")
        )

    # --- Dependencies (finish-to-start predecessors) -----------------------
    def _would_cycle(self, obj_id: str, pred_id: str) -> bool:
        """True if making pred_id a predecessor of obj_id creates a cycle."""
        seen: set[str] = set()
        stack = [pred_id]
        while stack:
            current = stack.pop()
            if current == obj_id:
                return True
            if current in seen:
                continue
            seen.add(current)
            node = self.model.objects.get(current)
            if node is None:
                continue
            for next_id in getattr(node, "predecessors", []) or []:
                stack.append(next_id)
        return False

    def can_add_predecessor(self, obj_id: str, pred_id: str) -> bool:
        """Return whether a finish-to-start dependency can be added."""
        obj = self.model.objects.get(obj_id)
        pred = self.model.objects.get(pred_id)
        unsupported = {"link", "connector", "arrow", "textbox"}
        if (
            obj is None
            or pred is None
            or obj_id == pred_id
            or obj.kind in unsupported
            or pred.kind in unsupported
        ):
            return False
        if pred_id in (obj.predecessors or []):
            return False
        return not self._would_cycle(obj_id, pred_id)

    def add_predecessor(self, obj_id: str, pred_id: str) -> bool:
        if not self.can_add_predecessor(obj_id, pred_id):
            return False
        obj = self.model.objects[obj_id]
        self.update_object(
            obj_id,
            {"predecessors": list(obj.predecessors or []) + [pred_id]},
            "Add Dependency",
        )
        return True

    def remove_predecessor(self, obj_id: str, pred_id: str) -> None:
        obj = self.model.objects.get(obj_id)
        if obj is None:
            return
        new_list = [pid for pid in (obj.predecessors or []) if pid != pred_id]
        if new_list != list(obj.predecessors or []):
            self.update_object(obj_id, {"predecessors": new_list}, "Remove Dependency")

    def remove_predecessors(
        self,
        relationships,
        description: str = "Remove Dependencies",
    ) -> None:
        """Remove several dependency relationships with one update per successor."""
        removals: dict[str, set[str]] = {}
        for successor_id, predecessor_id in relationships:
            removals.setdefault(successor_id, set()).add(predecessor_id)
        changes = []
        for successor_id, predecessor_ids in removals.items():
            obj = self.model.objects.get(successor_id)
            if obj is None:
                continue
            old_list = list(obj.predecessors or [])
            new_list = [pred_id for pred_id in old_list if pred_id not in predecessor_ids]
            if new_list != old_list:
                changes.append((successor_id, new_list))
        if not changes:
            return
        self.undo_stack.beginMacro(description)
        try:
            for successor_id, new_list in changes:
                self.update_object(
                    successor_id,
                    {"predecessors": new_list},
                    "Remove Dependency",
                )
        finally:
            self.undo_stack.endMacro()

    def set_auto_reschedule(self, enabled: bool) -> None:
        if self.model.dependencies_auto_reschedule == bool(enabled):
            return
        self.model.dependencies_auto_reschedule = bool(enabled)
        self.model.metadata_changed.emit()

    def add_divider(
        self,
        name: str = "Divider",
        *,
        index: int | None = None,
        description: str = "Add Divider",
    ) -> Topic:
        # Dividers reuse Topic with kind="divider"; layout renders them thin.
        topic = Topic(id=new_id(), name=name, color="#c7d2de", kind="divider")
        self.undo_stack.push(
            AddTopicCommand(self.model, topic, index=index, description=description)
        )
        return topic

    def update_classification(self, text: str, size: int | None = None) -> None:
        new_text, new_size = self.model.normalize_classification(text, size)
        old_text = self.model.classification
        old_size = self.model.classification_size
        if new_text == old_text and new_size == old_size:
            return
        self.undo_stack.push(
            UpdateClassificationCommand(self.model, old_text, old_size, new_text, new_size)
        )

    def update_overlap_tolerance(self, value) -> None:
        value = self.model.normalize_overlap_tolerance(value)
        if value != self.model.symbol_overlap_tolerance_percent:
            self.undo_stack.push(UpdateOverlapToleranceCommand(self.model, value))

    def update_topic(self, topic: Topic) -> None:
        current = self.model.get_topic(topic.id)
        if current is None or current == topic:
            return
        self.undo_stack.push(UpdateTopicCommand(self.model, current, topic))

    def update_deliverable(self, deliverable: Deliverable) -> None:
        found = self.model.find_deliverable(deliverable.id)
        if found is None:
            return
        _topic, _index, current = found
        if current == deliverable:
            return
        self.undo_stack.push(UpdateDeliverableCommand(self.model, current, deliverable))

    def add_deliverable(
        self,
        topic_id: str,
        name: str,
        *,
        index: int | None = None,
        indent: int = 0,
        description: str = "Add Deliverable",
    ) -> Deliverable | None:
        topic_index = self._topic_index(self.model.topics, topic_id)
        if topic_index is None:
            return None
        try:
            indent_value = int(indent)
        except (TypeError, ValueError):
            indent_value = 0
        deliverable = Deliverable(id=new_id(), name=name, indent=max(0, indent_value))
        old_topics = deepcopy(self.model.topics)
        new_topics = deepcopy(self.model.topics)
        topic = new_topics[topic_index]
        target_index = len(topic.deliverables) if index is None else max(
            0, min(index, len(topic.deliverables))
        )
        topic.deliverables.insert(target_index, deliverable)
        new_topics = self._normalize_topics(new_topics, {topic_id})
        if new_topics == old_topics:
            return None
        self.undo_stack.push(
            ReplaceTopicsCommand(self.model, old_topics, new_topics, description)
        )
        return deliverable

    def toggle_topic_collapsed(self, topic_id: str) -> None:
        topic = self.model.get_topic(topic_id)
        if topic is None:
            return
        self.undo_stack.push(ToggleTopicCollapseCommand(self.model, topic_id, topic.collapsed))

    def move_deliverable(self, deliverable_id: str, direction: int) -> bool:
        return self.move_deliverables([deliverable_id], direction)

    def move_deliverables(self, deliverable_ids: list[str], direction: int) -> bool:
        if direction == 0:
            return False
        ordered_ids = self._ordered_deliverable_ids(self.model.topics, deliverable_ids)
        if not ordered_ids:
            return False
        processing_ids = ordered_ids if direction < 0 else list(reversed(ordered_ids))
        old_topics = deepcopy(self.model.topics)
        new_topics = deepcopy(self.model.topics)
        affected_topic_ids: set[str] = set()
        moved = False
        for deliverable_id in processing_ids:
            affected = self._move_deliverable_in_topics(new_topics, deliverable_id, direction)
            if not affected:
                continue
            affected_topic_ids.update(affected)
            moved = True
        if not moved:
            return False
        new_topics = self._normalize_topics(new_topics, affected_topic_ids)
        description = "Move Deliverables" if len(ordered_ids) > 1 else "Move Deliverable"
        self.undo_stack.push(
            ReplaceTopicsCommand(self.model, old_topics, new_topics, description)
        )
        return True

    def move_deliverables_to(self, deliverable_ids: list[str], topic_id: str, index: int) -> bool:
        selected = set(deliverable_ids)
        old_topics = deepcopy(self.model.topics)
        new_topics = deepcopy(self.model.topics)
        target = next((topic for topic in new_topics if topic.id == topic_id), None)
        if target is None:
            return False
        moving = []
        for topic in new_topics:
            kept = []
            for deliverable in topic.deliverables:
                if deliverable.id in selected:
                    moving.append(deliverable)
                else:
                    kept.append(deliverable)
            topic.deliverables = kept
        if not moving:
            return False
        ordered = self._ordered_deliverable_ids(old_topics, selected)
        by_id = {deliverable.id: deliverable for deliverable in moving}
        moving = [by_id[deliverable_id] for deliverable_id in ordered if deliverable_id in by_id]
        target.deliverables[index:index] = moving
        affected = {topic.id for topic in new_topics if topic.id == topic_id or topic.id in {t.id for t in old_topics if any(d.id in selected for d in t.deliverables)}}
        new_topics = self._normalize_topics(new_topics, affected)
        if new_topics == old_topics:
            return False
        description = "Move Deliverables" if len(moving) > 1 else "Move Deliverable"
        self.undo_stack.push(ReplaceTopicsCommand(self.model, old_topics, new_topics, description))
        return True

    def remove_deliverable(self, deliverable_id: str) -> bool:
        return self.remove_deliverables([deliverable_id])

    def remove_deliverables(self, deliverable_ids: list[str]) -> bool:
        selected_ids = set(deliverable_ids)
        if not selected_ids:
            return False
        old_topics = deepcopy(self.model.topics)
        new_topics = deepcopy(self.model.topics)
        affected_topic_ids: set[str] = set()
        for topic in new_topics:
            original_count = len(topic.deliverables)
            topic.deliverables[:] = [
                deliverable
                for deliverable in topic.deliverables
                if deliverable.id not in selected_ids
            ]
            if len(topic.deliverables) != original_count:
                affected_topic_ids.add(topic.id)
        if not affected_topic_ids:
            return False
        new_topics = self._normalize_topics(new_topics, affected_topic_ids)
        removed_objects = self._objects_for_rows(selected_ids)
        description = "Delete Deliverables" if len(selected_ids) > 1 else "Delete Deliverable"
        self.undo_stack.push(
            ReplaceTopicsCommand(
                self.model,
                old_topics,
                new_topics,
                description,
                removed_objects=removed_objects,
            )
        )
        return True

    def adjust_deliverable_indent(self, deliverable_ids: list[str], delta: int) -> bool:
        if delta == 0:
            return False
        selected_ids = set(deliverable_ids)
        if not selected_ids:
            return False
        old_topics = deepcopy(self.model.topics)
        new_topics = deepcopy(self.model.topics)
        affected_topic_ids: set[str] = set()
        for topic in new_topics:
            topic_changed = False
            updated_deliverables: list[Deliverable] = []
            for deliverable in topic.deliverables:
                if deliverable.id not in selected_ids:
                    updated_deliverables.append(deliverable)
                    continue
                updated_deliverables.append(
                    replace(deliverable, indent=max(0, deliverable.indent + delta))
                )
                topic_changed = True
            if topic_changed:
                topic.deliverables[:] = updated_deliverables
                affected_topic_ids.add(topic.id)
        if not affected_topic_ids:
            return False
        new_topics = self._normalize_topics(new_topics, affected_topic_ids)
        if new_topics == old_topics:
            return False
        description = "Indent Deliverables" if delta > 0 else "Outdent Deliverables"
        self.undo_stack.push(
            ReplaceTopicsCommand(self.model, old_topics, new_topics, description)
        )
        return True

    def remove_topic(self, topic_id: str) -> bool:
        topic_index = None
        topic = None
        for index, entry in enumerate(self.model.topics):
            if entry.id == topic_id:
                topic_index = index
                topic = entry
                break
        if topic is None or topic_index is None:
            return False
        row_ids = {topic.id, *[d.id for d in topic.deliverables]}
        removed_objects = self._objects_for_rows(row_ids)
        self.undo_stack.push(RemoveTopicCommand(self.model, topic, topic_index, removed_objects))
        return True

    def move_topic(self, topic_id: str, direction: int) -> bool:
        if direction == 0:
            return False
        topic_index = self._topic_index(self.model.topics, topic_id)
        if topic_index is None:
            return False
        new_index = topic_index - 1 if direction < 0 else topic_index + 1
        if new_index < 0 or new_index >= len(self.model.topics):
            return False
        old_topics = deepcopy(self.model.topics)
        new_topics = deepcopy(self.model.topics)
        topic = new_topics.pop(topic_index)
        new_topics.insert(new_index, topic)
        if new_topics == old_topics:
            return False
        self.undo_stack.push(
            ReplaceTopicsCommand(self.model, old_topics, new_topics, "Move Topic")
        )
        return True

    def _objects_for_rows(self, row_ids: set[str]) -> list[CanvasObject]:
        row_objects = [
            obj
            for obj in self.model.objects.values()
            if obj.kind != "link" and (obj.row_id in row_ids or (obj.target_row_id in row_ids))
        ]
        row_object_ids = {obj.id for obj in row_objects}
        link_objects = [
            obj
            for obj in self.model.objects.values()
            if obj.kind == "link"
            and (
                (obj.link_source_id in row_object_ids)
                or (obj.link_target_id in row_object_ids)
            )
        ]
        connector_objects = [
            obj
            for obj in self.model.objects.values()
            if obj.kind in ("connector", "arrow")
            and (
                (obj.connector_source_id in row_object_ids)
                or (obj.connector_target_id in row_object_ids)
            )
        ]
        return row_objects + link_objects + connector_objects

    def objects_for_rows(self, row_ids: set[str]) -> list[CanvasObject]:
        return list(self._objects_for_rows(set(row_ids)))

    def make_default_object(
        self,
        kind: str,
        row_id: str,
        start_week: int,
        end_week: int | None = None,
        color: str | None = None,
    ) -> CanvasObject:
        if end_week is None:
            end_week = start_week
        if color is None:
            if kind == "deadline":
                color = DEADLINE_DEFAULT_COLOR
            else:
                topic = self.model.topic_for_row(row_id)
                color = topic.color if topic else DEFAULT_TOPIC_COLORS[0]
        size = DEFAULT_SIZE
        if kind == "box":
            size = 5
        if kind in ("arrow", "connector"):
            # Unified connectors use the connector defaults; the week range
            # becomes the free-mode start/target geometry.
            color = self.connector_default_color
            size = self.connector_default_size
        obj = CanvasObject(
            id=new_id(),
            kind=kind,
            row_id=row_id,
            start_week=start_week,
            end_week=end_week,
            text="",
            color=color,
            size=size,
            z_index=self._next_z_index(),
        )
        if kind in ("arrow", "connector"):
            obj = replace(obj, target_row_id=row_id, target_week=end_week)
        return self._normalize_object(obj)

    def make_textbox(self, x: float, y: float, width: float, height: float) -> CanvasObject:
        obj = CanvasObject(
            id=new_id(),
            kind="textbox",
            row_id=CANVAS_ROW_ID,
            start_week=0,
            end_week=0,
            text="",
            text_align="left",
            color=TEXTBOX_DEFAULT_COLOR,
            size=DEFAULT_SIZE,
            z_index=self._next_z_index(),
            x=x,
            y=y,
            width=width,
            height=height,
            opacity=TEXTBOX_NEW_OPACITY,
        )
        return self._normalize_object(obj)

    def add_anchor_link(
        self, source_id: str, target_id: str, side: str | None, offset: float | None
    ) -> None:
        source = self.model.objects.get(source_id)
        target = self.model.objects.get(target_id)
        if (
            source is None
            or target is None
            or source.kind != "textbox"
            or target.kind in ("link", "textbox", "connector", "arrow")
            or source_id == target_id
        ):
            return
        # A textbox may have many outgoing links, but the same target should
        # not receive duplicate paths from that textbox.
        if any(
            link.link_target_id == target_id
            for link in self._links_from_source(source_id)
        ):
            return
        if self.layout is None:
            return
        side_value = side if side in ("left", "right", "top", "bottom") else "right"
        offset_value = float(offset or 0.5)
        if offset_value < 0.0:
            offset_value = 0.0
        if offset_value > 1.0:
            offset_value = 1.0
        target_point = self._object_anchor_point(target)
        if target_point is None:
            return
        source_anchor = self._textbox_anchor_point(source, side_value, offset_value)
        link_offset_x = source_anchor[0] - target_point[0]
        link_offset_y = source_anchor[1] - target_point[1]
        z_index = min(source.z_index, target.z_index) - 1
        if z_index == 0:
            z_index = -1
        link_obj = CanvasObject(
            id=new_id(),
            kind="link",
            row_id=CANVAS_ROW_ID,
            start_week=0,
            end_week=0,
            text="",
            color=LINK_LINE_COLOR,
            size=DEFAULT_SIZE,
            z_index=z_index,
            link_source_id=source_id,
            link_target_id=target_id,
            link_source_side=side_value,
            link_source_offset=offset_value,
            link_offset_x=link_offset_x,
            link_offset_y=link_offset_y,
        )
        self.undo_stack.beginMacro("Anchor Textbox")
        self.add_object(link_obj, "Add Anchor")
        self.undo_stack.endMacro()

    def add_connector_arrow(
        self,
        source_id: str,
        target_id: str,
        source_side: str | None,
        source_offset: float | None,
        target_side: str | None,
        target_offset: float | None,
    ) -> None:
        source = self.model.objects.get(source_id)
        target = self.model.objects.get(target_id)
        if (
            source is None
            or target is None
            or source_id == target_id
            or source.kind in ("link", "connector", "arrow")
            or target.kind in ("link", "connector", "arrow")
        ):
            return
        source_side_value = (
            source_side if source_side in ("left", "right", "top", "bottom") else "right"
        )
        target_side_value = (
            target_side if target_side in ("left", "right", "top", "bottom") else "left"
        )
        source_offset_value = float(source_offset or 0.5)
        source_offset_value = max(0.0, min(1.0, source_offset_value))
        target_offset_value = float(target_offset or 0.5)
        target_offset_value = max(0.0, min(1.0, target_offset_value))
        z_index = min(source.z_index, target.z_index) - 1
        if z_index == 0:
            z_index = -1
        connector_obj = CanvasObject(
            id=new_id(),
            kind="connector",
            row_id=CANVAS_ROW_ID,
            start_week=0,
            end_week=0,
            text="",
            color=self.connector_default_color,
            size=self.connector_default_size,
            z_index=z_index,
            connector_source_id=source_id,
            connector_target_id=target_id,
            connector_source_side=source_side_value,
            connector_source_offset=source_offset_value,
            connector_target_side=target_side_value,
            connector_target_offset=target_offset_value,
        )
        self.add_object(connector_obj, "Add Connector")

    def reorder_objects(self, obj_ids: list[str], action: str) -> None:
        selected = [obj_id for obj_id in obj_ids if obj_id in self.model.objects]
        if not selected:
            return
        ordered_ids = self._ordered_object_ids()
        selected_set = set(selected)

        if action == "front":
            new_order = [obj_id for obj_id in ordered_ids if obj_id not in selected_set] + [
                obj_id for obj_id in ordered_ids if obj_id in selected_set
            ]
            self._apply_z_order(new_order, "Bring to Front")
            return
        if action == "back":
            new_order = [obj_id for obj_id in ordered_ids if obj_id in selected_set] + [
                obj_id for obj_id in ordered_ids if obj_id not in selected_set
            ]
            self._apply_z_order(new_order, "Send to Back")
            return
        if action == "forward":
            order = list(ordered_ids)
            for index in range(len(order) - 2, -1, -1):
                if order[index] in selected_set and order[index + 1] not in selected_set:
                    order[index], order[index + 1] = order[index + 1], order[index]
            self._apply_z_order(order, "Bring Forward")
            return
        if action == "backward":
            order = list(ordered_ids)
            for index in range(1, len(order)):
                if order[index] in selected_set and order[index - 1] not in selected_set:
                    order[index], order[index - 1] = order[index - 1], order[index]
            self._apply_z_order(order, "Send Backward")

    def _ordered_object_ids(self) -> list[str]:
        if not self.model.objects:
            return []
        order_index = {obj_id: index for index, obj_id in enumerate(self.model.objects.keys())}
        ordered = sorted(
            self.model.objects.values(),
            key=lambda obj: (obj.z_index, order_index.get(obj.id, 0)),
        )
        return [obj.id for obj in ordered]

    def _apply_z_order(self, ordered_ids: list[str], description: str) -> None:
        if not ordered_ids:
            return
        self.undo_stack.beginMacro(description)
        for index, obj_id in enumerate(ordered_ids):
            obj = self.model.objects.get(obj_id)
            if obj is None or obj.z_index == index:
                continue
            self.update_object(obj_id, {"z_index": index}, "Reorder")
        self.undo_stack.endMacro()
